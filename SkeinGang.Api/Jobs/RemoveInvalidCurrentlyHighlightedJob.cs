...
internal class RemoveInvalidCurrentlyHighlightedJob(
    HighlightService highlight,
    ILogger<RemoveInvalidCurrentlyHighlightedJob> logger
) : IJob
{
    ...
    public Task Execute(IJobExecutionContext context)
    {
        logger.LogInformation("Beginning scheduled removal job {Job} at {UtcNow}.",
            nameof(RemoveInvalidCurrentlyHighlightedJob), DateTime.UtcNow);
