...
internal class TeamFilter(Expression<Func<Domain.Team, bool>> filter)
{
    ...
    {
        private const int UntilTimeDefaultOffset = 30;

        internal TimesOfDay(LocalTime from, LocalTime? to = null) :
            this(from, to ?? from.PlusMinutes(UntilTimeDefaultOffset))
        {
        }
    }
    ...
    internal sealed class Archived(bool value) :
        TeamFilter(t => t.IsArchived == value);
}